...
$query="select * from tbl_dealer where username='$username'";
...
<table style="margin-left: -82px;height: 283px;">
    ...
    if(isset($_POST["update"])){
      ...
      $email=$_POST["email"];
      $phone=$_POST["phone"];
      $query="update tbl_dealer set name='$name',email='$email',phone='$phone' where username='$username'";
      $insert=mysqli_query($con,$query);
      if($insert){
        ?><script>
        window.location.href="dealerprofile.php"
        </script>
        <?php
      }
    }
